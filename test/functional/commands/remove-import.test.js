...
describe('[functional] crimesync remove-import command', () => {
  ...
  context('remove with a no-existent ID', () => {
    it('should raise a error', async () => {
      const ret = await crimeSync(['remove-import', '123456789012345678901234']);
      assert.equal(ret.stderr, 'Import \'123456789012345678901234\' not found.\n');
    });
  });

  context('remove a existent import', () => {
    it('should remove the import successfuly', async () => {
      const inserted = await crimeSync(['import', 'http://domain.com/test-file.pdf']);
      
      assert.notEqual(inserted.stdout, '', 'the stdout should have a content. may be the test-file.pdf is already imported.');
