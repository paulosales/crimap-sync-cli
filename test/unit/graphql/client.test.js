  ...
  context("with no config setted.", () => {
    context("with user setting the right url", () => {
      it("should request the service url from user and persist it.", async () => {
        ...
        chai.expect(setConfigSpy.calledWith("serviceUrl", SERVICE_URL)).to.be
          .true;
        chai.expect(saveConfigStub.calledOnce).to.be.true;
        chai.expect(
          writeStdOutStub.calledWith(`Service url ${SERVICE_URL} saved.`)
        );
      });
    });

    context(
      "with user setting a wrong url at first time and fixing at the second time",
      () => {
        it("should send a message to user fix the url and at the second moment it should persist the correct url.", async () => {
          ...
          chai.expect(
            writeStdOutStub.firstCall.calledWith(
              `The service URL ${WRONG_SERVICE_URL} is not valid or the service is not available.\n`
            )
          ).to.be.true;

          chai.expect(
            writeStdOutStub.secondCall.calledWith(
              `Try again or type CTRL + C to cancel.\n`
            )
          ).to.be.true;

          chai.expect(
            writeStdOutStub.thirdCall.calledWith(
              `Service url ${SERVICE_URL} saved.\n`
            )
          ).to.be.true;
